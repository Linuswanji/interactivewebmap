// creating a map layer with its basemap
			var map=L.map('map').setView([0.00,35.00,],13);
				L.tileLayer("http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
				attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors, ENC222-0425/2017, Linus Kiboi'}).addTo(map);
			// the json data
			var info = [{
            "address": {
                "building": "1",
			...
			for (x in info) {
    var positions = L.latLng({
        lat: info[x].address.coord[1],
        lng: info[x].address.coord[0]
    });
    var date, necessities;
    necessities = `<p style="font-weight: bold;"> Restaurant name:${info[x].name}</p> <p style="font-weight:bold"> Cuisine:${info[x].cuisine} cuisine </p> <p><b>Street</b>: ${info[x].address.street}</p>`

    for (y = 0; y < info[x].grades.length; y++) {
        date = "<br><span style='font-weight: bold'>Date: </span>" + info[x].grades[y].date.$date;
        necessities = necessities + date;
    };
    var points = L.marker(positions, {
        icon: icon
    }).bindPopup(necessities);
